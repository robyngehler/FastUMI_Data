...
- The recorder now uses three explicit phases per episode:
  1. Press Enter to start T265 tracking-only mode.
  2. Move into the record-init/start pose and press Enter again to start the episode recording.
  3. Press Enter a third time to actively stop, save, and process the episode.

- During collection, per-episode raw artifacts are archived under csv/temp_video_timestamps_<episode>.csv and csv/temp_trajectory_<episode>.csv, and Enter-triggered phase timestamps are logged in csv/episode_events.csv.
...
└── dataset/
    └── <task>/
        ...
        ├── csv/
        │   ├── episode_events.csv
        │   ├── temp_trajectory_0.csv
        │   ├── temp_video_timestamps_0.csv
        │   ├── temp_trajectory.csv
...
- episode_events.csv
  ...
  Purpose: Stores the Enter-triggered boundaries for each episode so downstream trimming tools can correlate tracking start, recording start, and recording stop with the archived raw CSV/video files.
