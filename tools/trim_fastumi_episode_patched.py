...
def get_frame_timestamp_bounds(
    *,
    ...
    video_path: Path,
    fps: float,
) -> tuple[Optional[float], Optional[float], Optional[pd.DataFrame], list[str]]:
    notes: list[str] = []
    ...
    idx_col = None
    for name in FRAME_TS_INDEX_COLUMNS:
        if name in frame_ts_df.columns:
            idx_col = name
            break
    ...
    if idx_col is not None:
        ...
        if row_pos >= len(frame_ts_df):
            raise IndexError(
                f"Episode index {episode_index} exceeds number of rows in {frame_ts_path} ({len(frame_ts_df)})"
            )

    start_ts = float(frame_ts_df.iloc[row_pos][ts_col])
    next_start_ts: Optional[float] = None
    if row_pos + 1 < len(frame_ts_df):
        next_start_ts = float(frame_ts_df.iloc[row_pos + 1][ts_col])
...
def segment_shared_episode_stream(
    *,
    df: pd.DataFrame,
    timestamp_col: str,
    start_ts: float,
    ...
    if out.empty:
        raise ValueError(
            f"Shared {kind} CSV could not be segmented for episode window "
            f"[{start_ts}, {end_ts if end_ts is not None else 'inf'})."
        )
    return out


def prepare_episode_streams(
    *,
    task_dir: Path,
    episode_index: int,
    video_path: Path,
    ...
    frame_ts_path: Optional[Path],
    episode_events_path: Optional[Path],
    video_fps_override: Optional[float],
) -> tuple[pd.DataFrame, pd.DataFrame, Optional[pd.DataFrame], float, list[str]]:
    ...
    start_ts, end_ts, episode_events_df, bound_notes = get_frame_timestamp_bounds(
    ...
    if start_ts is None:
        raise ValueError(
            "Shared temp CSV fallback was selected, but no frame_timestamps.csv or usable episode_events.csv "
            "was available to segment the shared stream per episode."
        )

    if shared_video_ts:
        episode_notes.append(f"Segmented shared video timestamp CSV {video_ts_path.name} for this episode.")
        raw_video_ts_df = segment_shared_episode_stream(
            df=raw_video_ts_df,
    ...
    if shared_traj:
        ...
        raw_traj_df = segment_shared_episode_stream(
            df=raw_traj_df,
            timestamp_col="Timestamp",
            start_ts=start_ts,
            end_ts=end_ts,
...
def get_hdf5_num_steps(in_hdf5: Path) -> int:
    with h5py.File(in_hdf5, "r") as src:
        ...
        if "observations" in src and "images" in src["observations"]:
            img_grp = src["observations"]["images"]
            for cam_name in img_grp.keys():
                candidates.append(int(img_grp[cam_name].shape[0]))
                break
        if "action" in src:
...
def trim_once(
    *,
    ...
    frame_ts_path: Optional[Path],
    episode_events_path: Optional[Path],
    video_ts_df: pd.DataFrame,
    traj_df: pd.DataFrame,
    episode_events_df: Optional[pd.DataFrame],
    ...
    if hdf5_path is not None and hdf5_path.exists():
        ...
        if len(downsampled_source) != hdf5_len:
            hdf5_notes.append(
                f"Downsampled source length ({len(downsampled_source)}) differs from HDF5 length ({hdf5_len}); "
                "indices were clipped to the valid HDF5 range when necessary."
            )
        if ds_keep.max() >= hdf5_len:
            clipped = ds_keep[ds_keep < hdf5_len]
            if clipped.size == 0:
                raise ValueError(
                    f"Trim requested 20 Hz indices up to {int(ds_keep.max())}, but HDF5 only has indices 0-{hdf5_len-1}. "
                    "This usually means the episode-local timestamp stream is longer than the HDF5 sequence."
                )
            hdf5_notes.append(
                f"Clipped {ds_keep.size - clipped.size} invalid 20 Hz indices because HDF5 only supports 0-{hdf5_len-1}."
            )
            ds_keep = clipped
    ...
    manifest_notes.extend(source_notes)
    manifest_notes.extend(hdf5_notes)
    manifest_notes.extend(states_notes)
...
def run_single_trim(
    *,
    ...
    prepared_video_ts_df, prepared_traj_df, episode_events_df, fps, source_notes = prepare_episode_streams(
        task_dir=task_dir,
        episode_index=episode_index,
        video_path=video_path,
        video_ts_path=video_ts_path,
    ...
    for window in windows:
        result = trim_once(
            ...
            frame_ts_path=frame_ts_path,
            episode_events_path=episode_events_path,
            video_ts_df=prepared_video_ts_df,
            traj_df=prepared_traj_df,
            episode_events_df=episode_events_df,
